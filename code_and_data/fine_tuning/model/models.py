...
class AutoEncoder(nn.Module):
    def __init__(self, model_name, config):
        ...
        self.embedd_size = int(config.get('encoder', 'embedding_size'))

    def forward(self, input_text):
        inputs = self.tokenizer(input_text, padding="longest", truncation=True, max_length=self.embedd_size, return_tensors='pt')
        for key in inputs:
    ...
    def search(self, query, corpus):
        ...
        # all_embedding_query = []
        # for i in query:
        #     inputs_f = self.tokenizer(i, padding='longest', truncation=True, max_length=128, return_tensors='pt')
        #     for ipt_key in inputs_f.keys():
        #         inputs_f[ipt_key] = inputs_f[ipt_key].cuda()
        #     embedding_f = self.bert(**inputs_f)
        #     output_f = self.pooler(inputs_f['attention_mask'], embedding_f)
        #     all_embedding_query.append(output_f)
        # file = open(f'query_mpnet_ler.pickle', 'wb')
        ...
        # all_embedding = []
        # for i in corpus:
        #     inputs_e = self.tokenizer(i, padding='longest', truncation=True, max_length=128,
        #                          return_tensors='pt')
        #     for ipt_key in inputs_e.keys():
        #         inputs_e[ipt_key] = inputs_e[ipt_key].cuda()
        #     embedding_e = self.bert(**inputs_e)
        #     output_e = self.pooler(inputs_e['attention_mask'], embedding_e)
        #     all_embedding.append(output_e)
        # file = open(f'string_scope_mpnet_ler.pickle', 'wb')
