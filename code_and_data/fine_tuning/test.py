...
from tools.test_tool import test,test_document
import pickle
...
def init_ours(config='./config/train/train.config', checkpoint=None):
    ...
    file = open(f'encoder.pickle','wb')
    pickle.dump(cross_case_cl.encoder_e, file)
...
def test_single_file(config, device, test_data, test_data_type, mode='test'):
    ...
    if config.getboolean('test', 'test_ours'):
        if config.get('test', 'test_specific') != "None": # specify testing model
